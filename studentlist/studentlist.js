...
const GOOGLE_APP_SCRIPT_URL = "https://script.google.com/macros/s/AKfycbysWlXcQa3X0OGXZiMS4CBivr7945cR-YujNTzinImZaA4ub519NdLCKRClt-nIkKS8uQ/exec"; 
...
async function updateSheetData(data, action, roll = null) {
    ...
    // Apps Script doPost ফাংশনের জন্য payload তৈরি করা হচ্ছে 
    const payload = {
        action: action, 
        data: data,     
        roll: roll      
    ...
    // UI Feedback এর জন্য বাটন নিয়ন্ত্রণ
    ...
    try {
        const response = await fetch(webAppUrl, {
            ...
            body: JSON.stringify(payload), // JSON স্ট্রিং হিসাবে পাঠানো
        });
        ...
        if (result.status === 'SUCCESS') {
            console.log(`Apps Script Success: ${result.message}`);
            return result; 
        } else {
            // Apps Script এর মধ্যে কোনো লজিক এরর হলে
            throw new Error(`Server Logic Error: ${result.message}`);
        }
        ...
        // বাটনকে আবার স্বাভাবিক অবস্থায় ফিরিয়ে আনা
        if (saveButton) {
...
function renderStudentList(students, headerText = null) {
    ...
    Object.keys(groupedByTeam).sort((a, b) => {
        ...
        switch (teamNumber) {
            case '1': 
                leader1Image = 'img/zahid.jpg'; 
                leader2Image = 'img/zahid.jpg'; 
                break;
            case '2': 
                leader1Image = 'img/zahid.jpg';
                leader2Image = 'img/zahid.jpg';
